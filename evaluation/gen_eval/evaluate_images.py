...
def main(args):
    ...
    subfolders = [
        x
        for x in os.listdir(args.imagedir)
        if os.path.isdir(os.path.join(args.imagedir, x)) and x.isdigit()
    ]
    ...
    for subfolder in subfolders:
        ...
        for imagename in os.listdir(os.path.join(folderpath, "samples")):
            ...
            if not os.path.isfile(imagepath) or not (
                re.match(r"\d+\.png", imagename) or re.match(r"\d+\.jpg", imagename)
            ):
                continue
